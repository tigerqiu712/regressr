...
class HTMLReportGenerator extends ReportGenerator {
  ...
  def writeAndGetFile(content: String) : Try[File] = Try {
    val fileName = s"testrun_${System.currentTimeMillis}.html"
  ...
  override def generate(reportEntries: Seq[ReportEntry]): Try[File] = Try {
    getContent(reportEntries).flatMap {
      content => writeAndGetFile(content)
    } match {
